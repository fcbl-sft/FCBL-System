...
const PPMeetingComponent: React.FC<PPMeetingProps> = ({ project, onUpdate, onBack }) => {
  ...
  const inputClass = "w-full border p-3 text-sm focus:border-green-600 outline-none font-normal text-black transition-all rounded bg-white";
  ...
  const remarkClass = "w-full border p-3 text-xs bg-white focus:border-green-600 transition-all min-h-[80px] resize-none outline-none font-normal rounded";

  return (
    <div className="flex flex-col h-full bg-gray-50 print:bg-white">
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center shrink-0 z-30 no-print">
        ...
        <div className="flex gap-2">
          ...
          {!isLocked && (
            <button onClick={createNewMeeting} className="flex items-center gap-2 btn-primary font-black shadow-md">
              <Plus className="w-4 h-4" /> Add New Record
            </button>
          )}
          {!isLocked && (
            <button onClick={handleManualSave} className="flex items-center gap-2 btn-primary font-black shadow-md transition-all">
      ...
      <div className="flex-grow flex overflow-hidden print:overflow-visible">
        ...
        <div className="w-64 bg-white border-r border-gray-200 flex flex-col shrink-0 no-print">
          <div className="p-4 border-b border-gray-100 font-black text-[10px] text-gray-400 uppercase tracking-widest bg-gray-50 flex justify-between items-center">
            Meeting History
            <button onClick={createNewMeeting} className="p-1 hover:bg-indigo-100 text-green-700 rounded transition-all"><Plus className="w-3.5 h-3.5" /></button>
          </div>
          <div className="flex-grow overflow-y-auto">
            {meetings.map(m => (
              <div
                ...
                className={`p-4 border-l-4 cursor-pointer transition-all flex justify-between items-center group ${activeMeetingId === m.id ? 'bg-green-50 border-l-green-600 shadow-inner' : 'border-l-transparent hover:bg-gray-50'}`}
              >
            ...
            <div className="w-full space-y-10 pb-32">
              ...
              <div className="bg-white p-8 border border-gray-200">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="p-2 fcbl-icon-box text-white rounded"><ClipboardList className="w-5 h-5" /></div>
              ...
              <div className="bg-white p-8 border border-gray-200">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="p-2 fcbl-icon-box text-white rounded"><Timer className="w-5 h-5" /></div>
                    <h3 className="text-sm font-black uppercase text-gray-800 tracking-widest">Production Planning</h3>
                  </div>
                  <button onClick={() => addToList(activeMeeting.id, 'productionDetails', { knittingStartDate: '', color: '', numMachines: 0, leadTimeDays: 0, productionPerDay: 0, remarks: '', attachments: [] })} className="p-2 btn-primary shadow-md transition-all"><Plus className="w-5 h-5" /></button>
                </div>
                <div className="space-y-6">
                  {activeMeeting.productionDetails.map((pd) => (
                    <div key={pd.id} className="p-6 bg-gray-50/50 border border-gray-100 group/row relative">
                      <button onClick={() => removeFromList(activeMeeting.id, 'productionDetails', pd.id)} className="absolute -top-2 -right-2 p-1.5 bg-white border border-gray-200 text-red-400 hover:text-red-600 opacity-0 group-hover/row:opacity-100 transition-all"><X className="w-4 h-4" /></button>
              ...
              <div className="bg-white p-8 border border-gray-200">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="p-2 fcbl-icon-box text-white rounded"><Calendar className="w-5 h-5" /></div>
                    <h3 className="text-sm font-black uppercase text-gray-800 tracking-widest">Key Milestones</h3>
                  </div>
                  <button onClick={() => addToList(activeMeeting.id, 'milestones', { label: '', date: '', remarks: '', attachments: [] })} className="p-2 btn-primary shadow-md transition-all"><Plus className="w-5 h-5" /></button>
                </div>
              ...
              <div className="bg-white p-8 border border-gray-200">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="p-2 fcbl-icon-box text-white rounded"><CheckCircle className="w-5 h-5" /></div>
                    <h3 className="text-sm font-black uppercase text-gray-800 tracking-widest">Official Approvals</h3>
                  </div>
                  <button onClick={() => addToList(activeMeeting.id, 'approvals', { name: '', date: '', signatureUrl: '', remarks: '', attachments: [] })} className="p-2 btn-primary shadow-md transition-all"><Plus className="w-5 h-5" /></button>
                </div>
              ...
              <div className="bg-white p-8 border border-gray-200">
                <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="p-2 fcbl-icon-box text-white rounded"><Ruler className="w-5 h-5" /></div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100">
                  <div className="flex justify-between items-center mb-4">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5 text-green-600" /> Measurement Notes / Observations</label>
                  </div>
                  <textarea
              ...
              <div className="bg-slate-900 p-8 rounded-[2rem] shadow-2xl text-white">
                ...
                <textarea
                  className="w-full bg-white border border-gray-200 rounded p-3 text-sm text-black font-normal min-h-[150px] outline-none focus:border-green-600 transition-all"
              ...
              <div className="mt-12 pt-10 border-t-2 border-slate-200 no-print">
                <h3 className="text-xl font-black uppercase tracking-tighter text-slate-800 mb-8 flex items-center gap-3">
                  <span className="w-8 h-8 fcbl-icon-box text-white rounded flex items-center justify-center text-xs">TP</span>
                  Technical Documentation & Feedback
                </h3>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {/* COMMENTS PANEL */}
                  <div className="bg-white p-8 rounded border border-gray-200 flex flex-col h-[500px]">
                    <div className="flex justify-between items-center mb-6">
                      <h4 className="font-black text-xs uppercase tracking-widest text-slate-400 flex items-center gap-2">
                        <MessageSquare className="w-4 h-4 text-green-600" /> Remarks Log
                      </h4>
                      <span className="text-[10px] font-black text-green-700 bg-green-50 px-2 py-1 rounded">{(activeMeeting.comments || []).length} Records</span>
                    </div>

                    <div className="flex-grow overflow-y-auto space-y-4 mb-6 pr-2">
                      {(activeMeeting.comments || []).map((c) => (
                        <div key={c.id} className="bg-gray-50 border border-gray-200 p-4 rounded relative group">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => startEditComment(c)} className="p-1 text-slate-400 hover:text-green-700"><Edit3 className="w-3.5 h-3.5" /></button>
                    ...
                    <div className="mt-auto">
                      <textarea
                        className="w-full bg-white border border-gray-200 rounded p-3 text-xs font-normal focus:border-green-600 outline-none mb-3 resize-none h-24"
                      ...
                      <button
                        ...
                        className="w-full btn-primary py-3 font-black text-[10px] tracking-widest flex items-center justify-center gap-2"
                      >
                  ...
                  <div className="bg-white p-8 rounded border border-gray-200 flex flex-col h-[500px]">
                    <div className="flex justify-between items-center mb-6">
                      <h4 className="font-black text-xs uppercase tracking-widest text-slate-400 flex items-center gap-2">
                        <Paperclip className="w-4 h-4 text-green-600" /> Reference Files
                      </h4>
                      <button
                        onClick={() => {
                          if (activeMeetingId) {
                        ...
                        className="p-2 bg-green-600 text-white rounded-xl shadow-md hover:bg-green-700 transition-all"
                      >
                    ...
                    <div className="flex-grow overflow-y-auto space-y-3 pr-2">
                      {(activeMeeting.infoAttachments || []).map((att) => (
                        <div key={att.id} className="flex items-center justify-between p-3 bg-gray-50 rounded border border-gray-200 group">
                          <div className="flex items-center gap-3 overflow-hidden">
                            <div className="p-2 fcbl-icon-box text-white rounded"><FileText className="w-5 h-5" /></div>
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => window.open(att.fileUrl)} className="p-2 text-green-700 hover:bg-white rounded-xl transition-all" title="Preview"><ExternalLink className="w-4 h-4" /></button>
                    ...
                    <div className="mt-8 p-4 bg-gray-50 rounded-2xl border border-green-100">
                      <p className="text-[9px] font-black uppercase text-green-600 tracking-tighter mb-2">Technical Note</p>
      ...
      {activeAttachmentTarget && activeAttachmentTarget.type !== 'GLOBAL_PP' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/60 backdrop-blur-sm no-print">
          <div className="bg-white w-full max-w-lg rounded shadow-xl flex flex-col overflow-hidden">
            ...
            <div className="p-6 bg-white border-t">
              <button
                ...
                className="w-full bg-green-600 text-white py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-green-700 transition-all flex items-center justify-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/70 backdrop-blur-md print:relative print:p-0 print:bg-white print:z-0">
          <div className="bg-white w-full max-w-6xl max-h-[95vh] rounded-[40px] shadow-2xl overflow-hidden flex flex-col print:max-w-none print:max-h-none print:shadow-none">
            <div className="p-8 border-b flex justify-between items-center bg-gray-50/50 no-print">
              <div className="flex items-center gap-4">
                <div className="p-3 fcbl-icon-box text-white rounded shadow-lg"><Users className="w-6 h-6" /></div>
                <h2 className="font-black text-2xl uppercase tracking-tighter">PP Meeting Summary Report</h2>
              </div>
              <div className="flex gap-2">
                <button onClick={handlePrint} className="flex items-center gap-2 bg-white border border-slate-300 text-slate-700 px-4 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-wider hover:bg-gray-50 transition-all">
                  <Printer className="w-4 h-4" /> Print
                </button>
                <button onClick={handleDownloadPDF} className="flex items-center gap-2 bg-white border border-slate-300 text-slate-700 px-4 py-2.5 rounded-2xl font-bold text-xs uppercase tracking-wider hover:bg-gray-50 transition-all">
                  <FileDown className="w-4 h-4" /> Download PDF
                </button>
                <button onClick={() => setShowPreview(false)} className="p-3 hover:bg-gray-200 rounded transition-all">
            ...
            <div id="pp-preview-content" className="p-12 overflow-y-auto bg-white flex-grow font-sans space-y-12 print:overflow-visible print:p-4">
              <div className="border-b-4 border-black pb-8 mb-10 flex justify-between items-end">
                <div>
                  ...
                  <p className="text-sm font-bold text-green-700 uppercase tracking-[0.2em]">{project.title} • POs: {(project.poNumbers || []).map(p => p.number).join(', ')}</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="space-y-4">
                  ...
                  <table className="w-full border-collapse border-2 border-black">
                    <thead className="fcbl-header-bar">
                      <tr>
                  ...
                  <div className="space-y-2">
                    {activeMeeting.milestones.map(ms => (
                      <div key={ms.id} className="flex justify-between items-center p-3 border-b-2 border-slate-100">
                        ...
                        <span className="text-[11px] font-black text-green-700">{ms.date || 'TBC'}</span>
                      </div>
                    ))}
                  </div>
                </div>
                            ...
                            {(activeMeeting.qcMeasurementTable?.groups || []).map((g: any) => {
                              ...
                              return (
                                <React.Fragment key={g.id}>
                                  ...
                                  <td className={`p-2 text-center border-r border-black font-black bg-green-50 ${(rowGroup.subColumns || []).some((sc: any) => checkTolerance(rowGroup.actualValue, sc.standardValue, row.tolerancePlus, row.toleranceMinus))
