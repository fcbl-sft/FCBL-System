...
const OrderSheetEditor: React.FC<OrderSheetEditorProps> = ({ project, onUpdate, onBack, onSave }) => {
  ...
  const inputClass = `w-full border p-3 text-sm focus:border-green-600 outline-none font-normal text-black transition-all rounded ${isLocked ? 'bg-gray-100 cursor-not-allowed' : 'bg-white'}`;
  ...
  return (
    <div className="flex flex-col h-full bg-white print:bg-white overflow-hidden" style={{ color: '#000000' }}>
      <header className="bg-white px-6 py-4 flex justify-between items-center shrink-0 z-30 no-print" style={{ borderBottom: '1px solid #E0E0E0' }}>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 fcbl-icon-box rounded" style={{ background: 'linear-gradient(90deg, #4CAF50, #388E3C)' }}>
      ...
      {showSaveToast && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 px-8 py-4 shadow-lg font-bold uppercase tracking-wider text-xs z-[100] flex items-center gap-3 rounded" style={{ background: 'linear-gradient(90deg, #4CAF50, #388E3C)', color: 'white' }}>
              ...
              <div className="bg-gray-50 p-6 border border-gray-200">
                ...
                <div className="overflow-x-auto border border-gray-200">
                  <table className="w-full">
                    <thead className="text-white" style={{ background: 'linear-gradient(90deg, #4CAF50, #388E3C)' }}>
                      ...
                      {(formData.poNumbers || []).map((po, idx) => (
                        <tr key={po.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          <td className="px-3 py-2">
                            <input
                              className="w-full bg-white border border-gray-200 px-3 py-2 text-sm font-bold text-black uppercase focus:border-green-600 outline-none"
                              value={po.number}
                          ...
                          <td className="px-3 py-2">
                            <input
                              type="number"
                              className="w-full bg-white border border-gray-200 px-3 py-2 text-sm font-bold text-center text-gray-700 focus:border-green-600 outline-none"
                              value={po.quantity || ''}
