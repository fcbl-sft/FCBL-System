...
const InspectionEditor: React.FC<InspectionEditorProps> = ({ project, inspection, onUpdate, onBack, onSave, onDeleteInspection }) => {
    ...
    const labelClass = "text-[10px] font-bold uppercase tracking-wider mb-1.5 flex items-center gap-1.5 pl-1";
    const commonInput = "w-full border p-3 text-sm focus:border-green-600 outline-none font-normal text-black transition-all rounded bg-white placeholder:text-gray-400";
    const tableInputClass = "w-full bg-white border border-gray-100 focus:border-green-600 p-2 text-xs outline-none font-normal text-black transition-all rounded";
        ...
        return (
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-100 no-print">
                <div className="flex items-center gap-3">
                    <div className={`p-2 transition-all ${isVisible ? 'fcbl-icon-box shadow-lg rounded' : 'bg-gray-100 text-gray-300'}`}>
        ...
        return (
            <div className="mt-8 pt-6 border-t border-gray-100 no-print">
                <div className="flex justify-between items-center mb-4">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><MessageSquare className="w-3.5 h-3.5 text-green-600" /> Section Remarks / Notas</label>
                    <button onClick={() => addComment(sectionId)} className="flex items-center gap-1.5 bg-green-50 text-green-700 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase hover:bg-indigo-100 transition-all"><Plus className="w-3 h-3" /> Add Note</button>
                </div>
                <div className="space-y-3">
                    {comments.map(c => (
                        <div key={c.id} className="flex flex-col gap-2 bg-gray-50 p-4 rounded border border-gray-200 group">
                            <div className="flex gap-2 items-start">
                                <input className="flex-grow bg-white border border-gray-200 rounded p-3 text-xs text-black font-normal focus:border-green-600 outline-none" value={c.text} onChange={e => updateComment(sectionId, c.id, e.target.value)} placeholder="Enter technical note..." />
                                <div className="flex gap-1 shrink-0">
                                    <button onClick={() => setActiveAttachmentTarget({ type: 'SECTION_COMMENT', targetId: c.id, sectionId })} className="p-2.5 bg-white text-slate-400 hover:text-green-700 rounded-xl transition-all relative border border-slate-200 shadow-sm">
                                        <Paperclip className="w-4 h-4" />
                                        {c.attachments?.length > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-green-600 text-white text-[8px] rounded-full flex items-center justify-center">{c.attachments.length}</span>}
                                    </button>
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 print:bg-white">
            <div className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center shrink-0 z-30 no-print">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="text-[10px] font-black uppercase text-green-600 tracking-widest">Style: {project.title} | POs: {(project.poNumbers || []).map(p => p.number).join(', ')}</div>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => setViewMode(viewMode === 'EDIT' ? 'PREVIEW' : 'EDIT')} className="flex items-center gap-2 bg-white border border-slate-300 text-slate-700 px-5 py-2.5 font-bold text-xs uppercase tracking-wider hover:bg-gray-50 transition-all">
                        {viewMode === 'EDIT' ? <Eye className="w-4 h-4" /> : <Edit className="w-4 h-4" />} {viewMode === 'EDIT' ? 'Report Preview' : 'Technical Editor'}
                    </button>
                    {!isLocked && (
                        <button onClick={handleManualSave} className="flex items-center gap-2 btn-primary px-5 py-2.5 font-black text-xs tracking-widest shadow-xl hover:bg-gray-800 transition-all">
            ...
            <div className="flex-grow flex overflow-hidden">
                <div className="w-72 bg-white border-r border-gray-200 flex flex-col shrink-0 no-print">
                    ...
                    {isAddingStage && (
                        <div className="p-4 border-b border-gray-100 bg-gray-50"><input autoFocus className="w-full border border-gray-200 p-2.5 text-xs mb-3 font-bold bg-white" value={newStageName} onChange={e => setNewStageName(e.target.value)} placeholder="e.g. Pre-Final Inspection" /><div className="flex gap-2"><button onClick={handleAddNewStage} className="flex-1 btn-primary p-2 text-[10px] font-black">Create</button><button onClick={() => setIsAddingStage(false)} className="flex-1 bg-white border border-slate-200 p-2 text-[10px] font-black uppercase">Cancel</button></div></div>
                    )}
                ...
                <div className="flex-grow overflow-y-auto p-8 bg-slate-100/30 print:p-0 print:bg-white relative">
                    <div className="w-full space-y-10 pb-32 no-print">
                        ...
                        {/* 1. GENERAL INFO */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('generalInfo') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                        ...
                        {/* 2. ORDER DETAILS */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('orderDetails') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                        ...
                        {/* 3. FACTORY INFO */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('factoryInfo') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                        ...
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('shipment') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            {renderSectionHeader('shipment', 'Quantities & Cartons Breakdown')}
                            <div className="space-y-6">
                                {data.shipmentGroups.map(group => (
                                    <div key={group.id} className="p-5 bg-gray-50 rounded border border-gray-200 group/group">
                                        <div className="flex justify-between items-center mb-5">
                                            <div className="flex items-center gap-4">
                                                <label className="text-[10px] font-black uppercase text-green-700">Color Group:</label>
                                                <input className="bg-white border border-gray-200 rounded p-2 text-xs font-normal w-40" value={group.color} onChange={e => updateField('shipmentGroups', data.shipmentGroups.map(g => g.id === group.id ? { ...g, color: e.target.value } : g))} />
                                            </div>
                                        ...
                                        <button onClick={() => updateField('shipmentGroups', data.shipmentGroups.map(g => g.id === group.id ? { ...g, rows: [...g.rows, { id: Date.now().toString(), size: 'M', orderQty: 0, shipQty: 0, cartonCount: 0 }] } : g))} className="mt-4 text-[9px] font-black text-green-700 uppercase tracking-widest hover:text-indigo-800 transition-colors flex items-center gap-1.5"><Plus className="w-3 h-3" /> Add Size Row</button>
                                    </div>
                                ))}
                                <button onClick={() => updateField('shipmentGroups', [...data.shipmentGroups, { id: Date.now().toString(), color: 'New Color', rows: [] }])} className="w-full py-4 border-2 border-dashed border-green-200 rounded text-green-600 font-bold text-[10px] uppercase tracking-wider hover:bg-green-50 hover:border-green-300 transition-all flex items-center justify-center gap-2"><PlusCircle className="w-4 h-4" /> Add New Color Group</button>
                            </div>
                            {renderComments('shipment')}
                        </div>

                        {/* 5. SAMPLING DATA */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('sampling') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                        ...
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('attachments') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            {renderSectionHeader('attachments', 'Attachments & Checklist')}
                            <button onClick={() => updateField('attachments', [...data.attachments, { id: Date.now().toString(), label: 'New Requirement', available: false, attachments: [], checked: false }])} className="mb-8 btn-primary text-[10px] font-black tracking-widest shadow-xl flex items-center gap-2 transition-all"><PlusCircle className="w-4 h-4" /> Add Requirement Item</button>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {data.attachments.map(att => (
                                    <div key={att.id} className={`p-5 border-2 transition-all duration-300 flex flex-col gap-4 ${att.available ? 'bg-gray-50 border-gray-200' : 'bg-slate-50 border-slate-100'}`}>
                                        <div className="flex justify-between items-start">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex flex-col">
                                                    <input className="text-xs font-bold uppercase text-black tracking-wider bg-white border border-gray-200 rounded p-1 mb-0.5 outline-none" value={att.label} onChange={e => updateField('attachments', data.attachments.map(a => a.id === att.id ? { ...a, label: e.target.value } : a))} />
                                        ...
                                        <button onClick={() => setActiveAttachmentTarget({ type: 'CHECKLIST', targetId: att.id })} className="w-full flex items-center justify-center gap-2 btn-primary py-2.5 text-[10px] font-black tracking-widest shadow-lg"><Paperclip className="w-3.5 h-3.5" /> Manage Documents</button>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* 7. QC DEFECTS */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('qcDefects') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            {renderSectionHeader('qcDefects', 'Quality Control (QC) Findings')}
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                                <div className="p-4 bg-gray-50 rounded border border-gray-200 text-center"><label className={labelClass}>Major Max Allowed</label><input type="number" className={`${commonInput} text-center`} value={data.qcSummary.maxAllowed || ''} onChange={e => updateField('qcSummary', { ...data.qcSummary, maxAllowed: parseInt(e.target.value) || 0 })} /></div>
                                <div className="p-4 bg-gray-50 rounded border border-gray-200 text-center"><label className={labelClass}>Critical Max Allowed</label><input type="number" className={`${commonInput} text-center`} value={data.qcSummary.criticalMaxAllowed || ''} onChange={e => updateField('qcSummary', { ...data.qcSummary, criticalMaxAllowed: parseInt(e.target.value) || 0 })} /></div>
                                <div className="p-4 bg-gray-50 rounded border border-gray-200 text-center"><label className={labelClass}>Minor Max Allowed</label><input type="number" className={`${commonInput} text-center`} value={data.qcSummary.minorMaxAllowed || ''} onChange={e => updateField('qcSummary', { ...data.qcSummary, minorMaxAllowed: parseInt(e.target.value) || 0 })} /></div>
                            </div>
                                ...
                                <table className="w-full border-collapse">
                                    <thead className="bg-slate-50">
                                        <tr>
                                            ...
                                            <th className="p-4 text-center text-[9px] font-black uppercase text-green-600 tracking-widest">Major</th>
                                    ...
                                    <tbody className="divide-y divide-slate-50">
                                        {data.qcDefects.map(defect => (
                                            <tr key={defect.id} className="hover:bg-slate-50/50 transition-colors">
                                                ...
                                                <td className="p-2"><input type="number" className={`${tableInputClass} text-center font-black text-green-700`} value={defect.major || ''} onChange={e => updateField('qcDefects', data.qcDefects.map(d => d.id === defect.id ? { ...d, major: parseInt(e.target.value) || 0 } : d))} /></td>
                                        ...
                                        <tr className="bg-gray-100 font-black border-t-2 border-slate-200">
                                            ...
                                            <td className="p-4 text-center text-green-700 text-sm">{defectTotals.major}</td>
                        ...
                        {/* 8. JUDGEMENT & RESULTS */}
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('judgement') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            ...
                            <div className="flex flex-col md:flex-row gap-10 items-center justify-center p-10 bg-slate-50/50 rounded-[2.5rem] border border-slate-100">
                                ...
                                <div className="flex-1 w-full max-w-lg">
                                    ...
                                    <textarea className="w-full h-32 bg-white border border-slate-300 rounded-3xl p-5 text-sm font-bold text-slate-900 focus:border-green-600 transition-all outline-none" value={data.judgementComments} onChange={e => updateField('judgementComments', e.target.value)} placeholder="General summary of the inspection findings and decision justification..."></textarea>
                                </div>
                            </div>
                        ...
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('measurements') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            {renderSectionHeader('measurements', 'Measurement Analysis')}

                            {/* Global Master Tolerance Control */}
                            <div className="flex flex-wrap items-center justify-between gap-6 mb-8 p-5 bg-gray-50 rounded border border-gray-200">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 fcbl-icon-box shadow-lg rounded"><Settings2 className="w-5 h-5" /></div>
                                    <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <input
                                        className="w-24 text-center py-2 bg-white border border-slate-300 text-sm font-black outline-none focus:border-green-600"
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex gap-2">
                                    ...
                                    }} className="btn-primary text-[10px] font-black tracking-widest shadow-xl flex items-center gap-2 transition-all"><PlusCircle className="w-4 h-4" /> Add Size</button>
                                    <button onClick={() => {
                                            ...
                                            {data.qcMeasurementTable.groups.map(g => (
                                                <th key={g.id} colSpan={g.colorCols.length + 1} className="p-2 text-center border-l border-slate-200">
                                                    <div className="flex items-center justify-center gap-2 mb-1">
                                                        <span className="text-[11px] font-black uppercase text-green-700 tracking-widest">{g.size}</span>
                                                        <button
                                                            onClick={() => {
                                                            ...
                                                            className="p-1 bg-white text-green-600 rounded border border-green-200 hover:text-green-700 transition-all"
                                        ...
                                        <tr>
                                            ...
                                            {data.qcMeasurementTable.groups.map(g => (
                                                <React.Fragment key={g.id}>
                                                    ...
                                                    <th className="p-2 text-center text-[8px] font-black uppercase text-green-600 tracking-widest border-l border-slate-200 bg-gray-50">Actual</th>
                                                </React.Fragment>
                                            ))}
                                        </tr>
                                    </thead>
                                                ...
                                                {data.qcMeasurementTable.groups.map(g => {
                                                    ...
                                                    return (
                                                        <React.Fragment key={g.id}>
                                                            ...
                                                            <td className="p-2 border-l border-slate-100 bg-green-50/30">
                        ...
                        <div className={`bg-white p-8 border border-gray-200 transition-all ${data.visibleSections.includes('images') ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {data.images.map((img, idx) => (
                                    <div key={idx} className="group relative bg-slate-50 border border-slate-100 overflow-hidden shadow-sm flex flex-col items-center">
                                        ...
                                        <div className="w-full p-4 bg-white border-t border-slate-50 flex gap-2">
                                            <input className="flex-1 bg-white border border-gray-200 rounded p-2 text-xs font-normal outline-none" value={img.label} onChange={e => updateField('images', data.images.map((im, i) => i === idx ? { ...im, label: e.target.value } : im))} />
                                ...
                                <button onClick={() => imageInputRef.current?.click()} className="h-56 border-4 border-dashed border-gray-200 flex flex-col items-center justify-center gap-3 text-gray-400 hover:text-black hover:bg-gray-50 hover:border-gray-300 transition-all group">
                                    <div className="p-4 bg-green-50 rounded-full group-hover:scale-110 transition-transform"><Camera className="w-8 h-8" /></div>
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/60 backdrop-blur-sm print:relative print:p-0 print:bg-white no-print">
                            <div className="bg-white w-full max-w-6xl max-h-[95vh] shadow-2xl overflow-hidden flex flex-col print:max-w-none print:max-h-none print:shadow-none">
                                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gray-50/50">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={handlePrint} className="flex items-center gap-2 btn-primary px-5 py-2.5 font-black text-xs tracking-widest shadow-xl">
            ...
            {/* SAVE TOAST */}
            {showSaveToast && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-green-600 text-white px-8 py-4 rounded shadow-lg font-bold uppercase tracking-wider text-xs z-[100] flex items-center gap-3">
                    ...
                    <div className="bg-white w-full max-w-lg shadow-2xl flex flex-col overflow-hidden">
                            ...
                            {getAttachmentsForTarget(activeAttachmentTarget).map(att => (
                                <div key={att.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 group">
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        <button onClick={() => window.open(att.fileUrl)} className="p-2 text-gray-400 hover:text-green-700 hover:bg-white rounded-lg transition-all"><ExternalLink className="w-4 h-4" /></button>
                        ...
                        <div className="p-6 bg-white border-t">
                            <button
                                ...
                                className="w-full btn-primary py-3 font-black tracking-widest shadow-xl flex items-center justify-center gap-2"
                            >
